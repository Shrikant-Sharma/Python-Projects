...
'''
Program: Commodity Data Filtering
...
Filename: prodExer-shri-02.py
Purpose: data handling of csv file using list comprehension
'''
# there are no literal constants
# there are no class definitions
# there are no function definitions
# title of the program
print('Program to print the handling of the data of csv file using list comprehension')
data = []
...
for row in reader:  # ! main loop reads one row at a time
    if reader.line_num == 1:  # ! get the location names from row 1
        ...
        # ! iterate through locations and values
        for location, value in zip(locations, row[2:]):
            ...
            data[row_num].append(
                float(value.replace('$', '')))  # ! append value
csvfile.close()
'''
...
print('There are', len(uComList), 'unique commodities')
####
...
comNums = [[x, comList.count(x)] for x in uComList]
####
# (5) for each item in comNums, print the number of records
# (two source code lines)
for item in comNums:

    print('Found', item[1], 'records for', item[0]+'.')
####
...
print(comNums[0][0], 'were available for', int((comNums[0][1])/5), 'weeks.')
####
...
comNumsDict = dict([[x, dict(comNums)[x]] for x in uComList])
####
# (15) Use comNumDict to print the number of weeks peaches were available for
# (one source code statement)
print('Peaches were available for', int((comNumsDict["Peaches"])/5), 'weeks.')
####
...
pAvg = sum([x[3] for x in data if x[0] == "Peaches" and x[2] == "Atlanta"]) / \
    len([x[3] for x in data if x[0] == "Peaches" and x[2] == "Atlanta"])
####
# (17) Print the average price of Peaches in Atlanta
print('The average price of peaches in Atlanta was', '$'+str(round(pAvg, 2))+'.')
####
'''
